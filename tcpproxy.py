...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
	...
	print(remote_host + ":" + str(remote_port))
...
#		print("After Hexdump\n")
		...
		if len(str(remote_buffer)):
			print ("[<==] Sending " + str(len(str(remote_buffer))) + " bytes to localhost.")
			client_socket.send(str(remote_buffer).encode())
...
#		print("After receive from: " + str(local_buffer))
...
def hexdump(src, length=16):
	...
	for i in range(0, len(src), length):
		...
		text = b''.join([bytes(x,"utf-8") if 0x20 <= ord(x) < 0x7F else b'.' for x in s])
...
def server_loop(local_host,local_port,remote_host,remote_port,receive_first):
	...
	try:
		...
		print ("[!!] Failed to listen on " + local_host + ":" + str(local_port))
